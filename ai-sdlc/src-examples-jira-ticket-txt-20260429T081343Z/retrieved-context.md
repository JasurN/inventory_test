...
Generate dry-run patch artifacts without editing the target repo:

```bash
npm run generate -- --repo-dir ../inventory-api --generate-patches --no-approval
```

Run validation and patch dry-run together:

```bash
npm run generate -- --repo-dir ../inventory-api --validate --generate-patches --no-approval
```
...
If requirements JSON does not match the schema, the Requirements Agent retries with validation feedback before failing. If repair fails after 3 total attempts, a failure report is written t
...
- [x] LangGraph orchestration parity
- [x] LangGraph durable checkpointing/resume
- [x] Codebase-aware patch dry-run generation
- [x] GitHub PR integration for generated SDLC artifacts
- [ ] Azure DevOps PR integration
- [x] Local CI validation integration
- [x] GitHub CI status/check-run capture
- [ ] Azure DevOps CI validation integration
...
> In enterprise SDLC, human review and auditability ar

[File truncated before retrieval scoring.]

---

## src/agents/code-patch-agent.ts

Score: 27

import { generateText } from "../core/llm.js";
import type { AgentContext, Requirements } from "../core/types.js";

export type CodePatchAgentInput = {
    ticket: string;
...
export type CodePatchOutput = {
    summary: string;
    patch: string;
};

export async function runCodePatchAgent(input: CodePatchAgentInput): Promise<CodePatchOutput> {
    const response = await generateText({
        system: `
You are a senior implementation engineer creating a dry-run patch proposal.

Return Markdown with exactly these sections:

# Patch Summary

Describe the intended implementation approach, likely files to change, test impact, and risks.

# Proposed Patch

Provide a unified diff proposal inside one fenced diff block.

Rules:
- Do not claim the patch was applied.
- Do not modify files.
- Prefer minimal, reviewable changes.
- Use only evidence from the ticket, plans, review, validation, and provided repo context.
- If exact file paths are unclear, produce a clearly marked illustrative patch and explain what must be confirmed by a human.
- Keep the patch realistic and focused on the requested feature.
        `.trim(),
        user: `
Original ticket:
...
${input.validationSummary ? `Validation results:\n\n${input.validationSummary}` : ""}

${input.agentContext ? `Repository/project context:\n\n${input.agentContext.content}` : ""}
        `.trim()
    });

    return splitPatchResponse(response);
}

function splitPatchResponse(response: string): CodePatchOutput {
    const patchMatch = response.match(/```diff\s*([\s\S]*?)```/i);
    const patch = patchMatch?.[1]?.trim();
    const summary = response
        .replace(/# Proposed Patch[\s\S]*$/i, "")
        .replace(/^# Patch Summary\s*/i, "")
        .trim();
